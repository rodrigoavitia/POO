...
class SplashView(ctk.CTkFrame):
    ...
    def iniciar_carga(self):
        ...
        if self.progreso_actual < 1.0:
            ...
            if self.progreso_actual > 0.5: self.lbl_status.configure(text="Conectando base de datos...")
            if self.progreso_actual > 0.7: self.lbl_status.configure(text="Finalizando...")
            if self.progreso_actual > 0.8: self.lbl_status.configure(text="Bienvenido")

            
            self.after(30, self.iniciar_carga) 
        else:
            # Carga completa: Ir al Login y poder empezar a usar el programa
